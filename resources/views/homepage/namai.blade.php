			...
			<div class="col-md-8 col-md-offset-2">
				...
				<div class="row">
					<div class="logo-wrapper">
						<img class="logo-lg" alt="DEMO" src="{{ asset('images/remapas-logo-big-clear.png') }}">
						<h2>{{trans('messages.homepage-line1')}}</h2>
					</div>
				</div>
				<div class="row">
					<div class="col-md-12">
						<div class="bg text-center homepage-line2 margin-bottom-15">
							<h4>{{trans('messages.homepage-line2')}}</h4>
						</div>
					</div>
				</div>
				<div class="row">
...
$(document).ready(function(){preload("747","748","749","751","752","753","754","755","756","757","758","759","760","761","762","763","764","765","766","767","768","769","770","771","772","773","774","775","776","777","778","779","780","781","782","783","784","785","786","787","788","789","790","791","792","793","794","795","796","797","798","799","800","801");function e(e,t,o){$.ajax({type:"POST",data:{id:t,list:e},url:'{{ route("ajax.data") }}',beforeSend:function(){$("."+e).html('<option selected="selected" disabled="disabled">{{trans("messages.loading")}}</option>')},success:function(t){null===o&&$("."+e).html("<option></option>"),jQuery.each(t,function(t,n){if(o==n.id){if(void 0!==n.name)a="<option data-id='"+n.item_id+"' selected='selected' value="+n.id+">"+n.name+"</option>";else if(void 0!==n.name_en)a="<option data-id='"+n.item_id+"' selected='selected' value="+n.id+">"+n.name_en+"</option>";$(a).appendTo("."+e)}else{if(void 0!==n.name)a="<option data-id='"+n.item_id+"' value="+n.id+">"+n.name+"</option>";else if(void 0!==n.name_en)var a="<option data-id='"+n.item_id+"' value="+n.id+">"+n.name_en+"</option>";$(a).appendTo("."+e)}}),$("."+e).select2("open")}})}$("#validateForm").validate({rules:{form_type:{required:!0},form_make:{required:!0},form_model:{required:!0},form_engine:{required:!0},form_name:{required:!0},form_email:{required:!0},form_text:{required:!0}},highlight:function(e){$(e).closest(".form-group").addClass("has-error")},unhighlight:function(e){$(e).closest(".form-group").removeClass("has-error"),$(e).closest(".form-group").addClass("has-success")},errorElement:"span",errorClass:"help-block",errorPlacement:function(e,t){t.parent(".input-group").length?e.insertAfter(t.parent()):e.insertAfter(t)},submitHandler:function(e){$.ajax({type:"POST",url:'{{route("contact")}}',data:$("#validateForm").serialize(),success:function(e){gtag('event', 'conversion', {'send_to': 'AW-869656606/znl0CO_r75kBEJ7Q154D'}),$("#cancelForm").trigger("click"),swal("Success!",e.success,"success")},error:function(e){var t=JSON.parse(e.responseText);swal("Error!",t.error,"error")}})}}),$.ajaxSetup({headers:{"X-CSRF-TOKEN":$('meta[name="csrf-token"]').attr("content")}}),$(".make").select2({width: '100%',placeholder:"{{trans('messages.make')}}",templateResult:function(e){var t=["747","748","749","751","752","753","754","755","756","757","758","759","760","761","762","763","764","765","766","767","768","769","770","771","772","773","774","775","776","777","778","779","780","781","782","783","784","785","786","787","788","789","790","791","792","793","794","795","796","797","798","799","800","801"];return e.id&&t.includes(e.id)?'<img class="option-logo" src="{{url("images/optlogos")}}/'+e.id+'.png" />'+e.text:e.text},templateSelection:function(e){return e.text},escapeMarkup:function(e){return e}}),$(".model").select2({width: '100%',placeholder:"{{trans('messages.model')}}"}),$(".engine").select2({width: '100%',placeholder:"{{trans('messages.engine')}}"}),$(".type").on("change",function(){$(".model").html("<option></option>"),$(".engine").html("<option></option>"),e("make",$(this).val(),null)}),$(".make").on("change",function(){$(".engine").html("<option></option>"),e("model",$(this).find(":selected").attr("data-id"),null)}),$(".model").on("change",function(){e("engine",$(this).find(":selected").attr("data-id"),null)}),$(".brand-logo").on("click",function(){$(".make").val($(this).attr("data-id")).trigger("change")}),$(".engine").change(function(){var e=$(this).find(":selected").attr("data-id"),t=$(".model").find(":selected").attr("data-id");$.ajax({type:"POST",data:{id:e,list:"747",model:t},url:'{{ route("ajax.meta") }}',beforeSend:function(){},error:function(e,t){alert('{{trans("messages.please-refresh-page")}}')},success:function(e){jQuery.each(e,function(e,t){$(".cartitle").css("background-image","url(../images/blogos/"+t.brand_logo+")"),$(".make-model-title").html($(".make option:selected").text()+" "+$(".model option:selected").text()),$(".engine-title").html($(".engine option:selected").text()),$(".formMake").val($(".make option:selected").text()),$(".formModel").val($(".model option:selected").text()),$(".formEngine").val($(".engine option:selected").text());var o=t.power,n=t.torque,a=t.stage1_power,r=t.stage1_torque,i=t.engine_type_id,l=Math.round((a-o)/(o/100));eco_up=1==i?10:2==i?10:3==i?15:4==i?13:15,2==i||4==i||5==i?$(".powerbox").hide():$(".powerbox").show();var s=Math.round(a-8);$(".more-par").attr("href","{{route('chiptuning')}}?make="+$(".make").val()+"&model="+$(".model").val()+"&engine="+$(".engine").val()),$(".carousel-render").hide(),$(".render").slideDown("slow"),$(".power_up").html(l),$(".eco_up").html(eco_up),$(".power").html(" "+o+"PS ("+Math.round(o/1.36)+"kW)"),$(".torque").html(" "+n+"Nm ("+Math.round(.7375621*n)+"ft-lb)"),$(".stage1_power").html(" "+a+"PS ("+Math.round(a/1.36)+"kW)"),$(".stage1_torque").html(" "+r+"Nm ("+Math.round(.7375621*r)+"ft-lb)"),$(".more_power").html(" "+Math.round(a-o)+"PS ("+Math.round((a-o)/1.36)+"kW)"),$(".more_torque").html(" "+Math.round(r-n)+"Nm ("+Math.round(.7375621*(r-n))+"ft-lb)"),$(".powerbox_power").html(" "+s+"hp ("+Math.round(s/1.36)+"kW)"),$(".more_eco").html(" {{trans('messages.up-to')}} "+eco_up+"%")})}})})});
</script>
@endsection
